...
//METODO 2b
	public List<Fermata> getFermateConnesse(Fermata partenza) {
		...
		final String sql = "SELECT id_fermata, nome, coordx, coordy FROM fermata"+
		        " WHERE id_fermata IN(SELECT id_stazA FROM connessione WHERE id_stazP=? GROUP BY id_stazA)"+
				" ORDER BY nome ASC";
		List<Fermata> fermate = new ArrayList<Fermata>();
...
//METODO 3
	public List<CoppiaId> getCoppiaFermateConnesse(){
		final String sql = "SELECT Distinct id_stazP, id_stazA FROM connessione";
		List<CoppiaId> coppiaId = new ArrayList<>();
